...
export class PageDesignerService {
  ...
  getControlObjectByType(controleType: string, menuId: string, parentControlId: string, controlName: string, order: number): IPageControl {
    let control;
    switch (controleType) {
      case PageResources.Controls.container: control = containerDirectoryObj.getNewInstance(menuId, parentControlId, controlName, order); break;
      case PageResources.Controls.textbox: control = textBoxDirectoryObj.getNewInstance(menuId, parentControlId, controlName, order); break;
      case PageResources.Controls.button: control = buttonDirectoryObj.getNewInstance(menuId, parentControlId, controlName, order); break;
    }
    return control;
  }
  ...
  addPageContainer() {
    ...
    const newContainer: ContainerControl = new ContainerControl(null, this.producMenuService.selectedMenuId, null, this.getNewControlName(PageResources.Controls.container), (this.containers.value.length + 1));
  ...
  deleteControl(controls: IPageControl[]): IActionResult {
    let result: IActionResult = {
      completed: false,
      message: PageResources.DeleteControlNotFound
    }
    if (this.selectedControl.value) {
      for (let i = 0; i < controls.length; i++) {
        if (!result.completed && controls[i].controlProperties.controlId === this.selectedControl.value.controlProperties.controlId) {
          ...
          result.message = PageResources.DeleteControlSuccessfully;
  ...
  private convertDBControlToObject(item: any): IPageControl {
    ...
    switch (props['controlType']) {
      case PageResources.Controls.container:
        ...
        break;
      case PageResources.Controls.textbox:
      ...
      case PageResources.Controls.button:
  ...
  private registerAddPageControlAction(node: IPageControl) {
    this.registerPageControlAction(node, PageResources.CrudActions.Add);
  }
  ...
  private registerUpdatePageControlAction(node: IPageControl) {
    ...
    if (controlNode) {
      ...
      this.registerPageControlAction(node, PageResources.CrudActions.Update, dbNode);
    }
  }

  private registerDeletePageControAction(node: IPageControl) {
    const isNewNode = (this.pageControlAction.findIndex(ma => ma.ControlItem.controlProperties.controlId == node.controlProperties.controlId && ma.ControlOperation === PageResources.CrudActions.Add) >= 0);
    this.deletePageControlAction(node);
    if (!isNewNode) {
      this.registerPageControlAction(node, PageResources.CrudActions.Delete);
    }
  }
  ...
  public getFormControlsFromDB(menuId: string): Observable<IPageControl[]> {
    return this.httpService.getData(PageResources.PageControlApiUrl.getAllMenuPageControls + '/' + menuId).pipe(
      map((d: any[]) => {
        this.controlsFromDB = [];
  ...
  public saveAllChanges(): Observable<any> {
    const addedRecords: IPageControlAction[] = this.pageControlAction.filter(ma => ma.ControlOperation === PageResources.CrudActions.Add);
    const updatedRecords: IPageControlAction[] = this.pageControlAction.filter(ma => ma.ControlOperation === PageResources.CrudActions.Update);
    const deletedRecords: IPageControlAction[] = this.pageControlAction.filter(ma => ma.ControlOperation === PageResources.CrudActions.Delete);
  ...
  private generateApiCalls(menuActions: IPageControlAction[]): Observable<any>[] {
    ...
    if (menuActions && menuActions.length > 0) {
      menuActions.forEach(ma => {
        ...
        if (ma.ControlOperation === PageResources.CrudActions.Add) {
          // ADD
          arr.push(this.httpService.postData(PageResources.PageControlApiUrl.addMenuPageControls, newItem));
        } else if (ma.ControlOperation === PageResources.CrudActions.Update) {
          // UPDATE
          arr.push(this.httpService.putData(PageResources.PageControlApiUrl.updateMenuPageControls + '/' + newItem.controlId, newItem));
        } else if (ma.ControlOperation === PageResources.CrudActions.Delete) {
          // DELETE
          arr.push(this.httpService.deleteData(PageResources.PageControlApiUrl.deleteMenuPageControls + '/' + newItem.controlId));
        }
      });
    }
    return arr;
